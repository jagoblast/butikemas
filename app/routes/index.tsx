...
// Fungsi penggabung class CSS (pengganti cn dari utils)
...
const dummyData = {
  ...
  articles: [
    { slug: 'tips-investasi', tag: 'Edukasi', title: '5 Tips Aman Investasi Emas untuk Pemula', desc: 'Pelajari cara membedakan emas asli dan palsu sebelum mulai berinvestasi jangka panjang.', coverUrl: 'https://emas.pasdigi.id/images/banner-1.png' },
    { slug: 'harga-naik', tag: 'Berita', title: 'Harga Emas Antam Sentuh Rekor Tertinggi Tahun Ini', desc: 'Ketegangan geopolitik dan inflasi global memicu lonjakan harga emas di kuartal ketiga.', coverUrl: 'https://emas.pasdigi.id/images/banner-2.png' },
    { slug: 'perak-alternatif', tag: 'Wawasan', title: 'Mengapa Perak Bisa Jadi Alternatif Investasi?', desc: 'Selain emas, perak juga memiliki nilai lindung nilai (hedging) yang baik saat ekonomi tidak stabil.', coverUrl: 'https://emas.pasdigi.id/images/banner-3.jpg' },
  ],
  features: [
    ...
    { icon: Package, title: 'Pengiriman Aman', desc: 'Layanan penuh se-Indonesia.' },
    { icon: Store, title: 'Butik Fisik', desc: 'Fasilitas pengambilan di butik resmi.' },
  ]
}
      ...
      <section className="overflow-hidden border-b border-gold-500/20 bg-navy-900">
        ...
        <div className="relative min-h-[calc(100svh-4rem)] overflow-hidden md:hidden">
          <video autoPlay loop muted playsInline className="absolute inset-0 h-full w-full object-cover object-center">
            {/* Menggunakan Video Eksternal Anda */}
                  ...
                  <div key={card.label} className={cn('relative overflow-hidden rounded-lg border p-5 shadow-sm', card.tone === 'gold' ? 'border-gold-400/60 bg-[linear-gradient(135deg,#fff7db_0%,#f3c85f_42%,#8b650f_120%)]' : 'border-slate-300 bg-[linear-gradient(135deg,#ffffff_0%,#dce1e8_48%,#748091_120%)]')}>
                    ...
                    <div className="relative">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <span className={cn('inline-flex min-w-[76px] items-center justify-center rounded-full px-3 py-1.5 text-xs font-bold', card.trend === 'up' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700')}>
                          {card.changeLabel}
                        </span>
                      </div>
              ...
              <article key={article.title} className="min-w-[85%] md:min-w-[30%] snap-center card-surface rounded-2xl overflow-hidden flex flex-col shadow-elevation-mid">
                <div className="relative aspect-video bg-gradient-to-br from-navy-900 to-navy-800 flex items-center justify-center overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4">
                    <span className="bg-navy-900/80 backdrop-blur-md text-gold-400 text-[8px] font-bold px-2 py-1 rounded uppercase tracking-wider">{article.tag}</span>
                  </div>
                </div>
                <div className="p-5 flex flex-col flex-grow">
                  ...
                  <a href={`/articles/${article.slug}`} className="flex items-center gap-1 font-bold text-gold-500 text-xs group">
