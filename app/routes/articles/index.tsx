            ...
            {articles.map((article: any) => (
              <a key={article.id} href={`/articles/${article.slug}`} className="block border border-gray-200 rounded-3xl overflow-hidden group hover:shadow-xl transition-all bg-white flex flex-col h-full">
                <div className="aspect-[16/10] overflow-hidden relative bg-navy-900">
                  ...
                  {/* Gunakan cover_url */}
                  {article.cover_url ? (
                    <img src={article.cover_url} alt={article.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700 opacity-90 group-hover:opacity-100" />
                  ) : (
                ...
                <div className="p-6 flex-1 flex flex-col">
                  ...
                  {/* Tampilkan Excerpt jika ada, kalau tidak fallback ke potongan content */}
